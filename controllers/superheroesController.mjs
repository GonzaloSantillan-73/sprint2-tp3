...
export async function obtenerSuperHeroePorIdController(req,res){
  try{
    ...
    const superheroFormateado = renderizarSuperheroe(superheroe)
    res.status(200).json(superheroFormateado)
  }
  catch(error){
...
export async function obtenerSuperheroesMayoresDe30Controller(req,res){
  try{
    ...
    res.status(500).send({ mensaje: "Error al obtener superheroes mayores de 30 años", error: error.message})
  }
}